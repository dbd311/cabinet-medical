 ...
 * The Osteo class
 *
 * The main Osteo class contains diferent elements to help you develop faster
 *
 */

class Osteo extends Controller
{
    public static function version()
    {
        return Osteo::settings()->osteo_version;
    }

    public static function websiteTitle()
    {
        return Osteo::settings()->website_title;
    }
    ...
    {
        return Osteo::role('id', Osteo::userSettings()->default_role);
    }
    ...
    {
        if(!Osteo::loggedInUser()->hasPermission($slug)) {
    ...
    {
        if(Osteo::document('name', $file_name)) {
            return true;
        } else {
            return false;
        }
    ...
    public static function downloadLink($file_name)
    {
        ...
        if(Osteo::isDocument($file_name)) {
            ...
            $link = route('Osteo::document_downloader', ['slug' => $document->slug]);
        }
        return $link;
    }
    ...
    public static function checkDocumentOwner($type, $data){
        if(Auth::user()->id == Osteo::document($type, $data)->author->id) {
            return true;
        } else {
            return false;
        }
    ...
    public static function dropdown($slug, $array = null)
    {
        ...
        if($array) {
            ...
            foreach($dropdowns[$slug] as $drop) {
                array_push($dropdowns_object, Osteo::rettype($drop, 'object'));
            }
    ...
    {
        if(Osteo::isAdmin($user) and !Osteo::loggedInUser()->su) {
    ...
    {
        return asset(Osteo::publicPath() . '/images/avatar.jpg');
    }

    public static function mustHaveBlog($blog_id)
    {
        if(!Osteo::loggedInUser()->su){
            if(!Osteo::loggedInUser()->has_blog($blog_id) and !Osteo::loggedInUser()->owns_blog($blog_id)){
    ...
    {
        if(!Osteo::loggedInUser()->su){
            if(!Osteo::loggedInUser()->owns_blog($blog_id)){
                abort(403, trans('laralum.error_not_allowed'));
            }
        }
    }
    ...
    {
        $id = Osteo::randomString();

        if(Osteo::settings()->pie_chart_source == 'chartjs') {
            $graph = "
            <canvas id='$id'></canvas>
            <script>
            ...
            ";
        } elseif(Osteo::settings()->pie_chart_source == 'highcharts') {
            $graph = "
    ...
    public static function barChart($title, $element_label, $labels, $data, $colors = null)
    {
        ...
        if(Osteo::settings()->bar_chart_source == 'chartjs') {
            $graph = "
            <canvas id='$id'></canvas>
            <script>
                var ctx = document.getElementById('$id');
            ...
            ";
        } elseif(Osteo::settings()->bar_chart_source == 'highcharts') {
            $graph = "
                <script type='text/javascript'>
                    $(function () {
                        $('#$id').highcharts({
    ...
    {
        $id = Osteo::randomString();
        $default_color = Osteo::settings()->header_color;

        if(Osteo::settings()->line_chart_source == 'chartjs') {
            $graph = "
            <canvas id='$id'></canvas>
            <script>
            	var ctx = document.getElementById('$id');
            ...
            ";
        } elseif(Osteo::settings()->line_chart_source == 'highcharts') {
            $graph = "
            <script type='text/javascript'>
                $(function () {
                    $('#$id').highcharts({
    ...
    public static function geoChart($title, $element_label, $data)
    {
        ...
        if(Osteo::settings()->geo_chart_source == 'highcharts') {
            $graph = "
                <script type='text/javascript'>
                    $(function () {
    ...
    {
        $json = file_get_contents(Osteo::publicPath() . '/assets/countries/names.json');
    ...
    {
        return asset(Osteo::publicPath() . '/images/logo-text.png');
    }
    ...
    {
        return app_path() . '/Http/Controllers/Osteo/Data';
    }
